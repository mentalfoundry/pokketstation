...
`0x0C800000`: `IRDA_MODE` (`+0x0`) bit0 `IFMODE` (0=Receive, 1=Transmit), bit1 `STDBY` (0=Active, 1=Stand-by), bit2 `BGEN` (0=Enable 40kHz carrier generator, 1=Disable), bit3 `BFLT` (0=Enable filter, 1=Disable). `IRDA_DATA` (`+0x4`) bit0 `LED`: in transmit, the value software bit-bangs (0=off, 1=on); in receive, this emulator's own read-back of the current demodulated level. That receive-side meaning is inferred, not confirmed. See below.
