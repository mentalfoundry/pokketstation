...
- **Unresolved: a real IR-using app's transmitted pulses come out ~1% too short in this emulator, and it is not yet known why.** The app arms Timer2 with its nominal pulse unit minus a hardcoded 184 (1200 − 184 = 1016), and its receive handler accepts a sync pulse only within `4×unit ± unit/2`, i.e. `[4200, 5400]` Timer2 ticks. This emulator produces 4162. That is short by 38, just outside the window, so no transfer ever decodes.
  ...
  **What is still unexplained: the receiver never writes its data buffer, and the transmit-stretch constant has no bearing on that.** Sweeping `IR_TX_FALL_STRETCH_CYCLES` across 0, 100, 200, 300 and 380 changes only whether the sync-pulse measurement lands in the app's acceptance window; every value produces the identical outcome downstream (same furthest-reached value of the receiver's `field+0x28`, the same 660 changes to it, and zero writes to the receive buffer). Even the value 0, which fails the sync window outright, behaves the same as values that pass it. That rules out this constant as the remaining blocker, and also undercuts reading `field+0x28` as a receive protocol state at all: 660 changes against 658 delivered edges is very close to one change per edge, which is the signature of a field tracking the live line level, not a state machine advancing through sync and bit-accumulation phases. Earlier notes here that described this field as a receive state machine, and its 1/2 oscillation as repeated failed sync locks, should be treated as unconfirmed inference from a guessed field offset rather than established fact. Also worth noting for whoever picks this up: the receiving instance never transmits anything back (0 edges in the reverse direction across every run), so if the real protocol expects any acknowledgement or handshake from the receiving side, nothing in these runs would have satisfied it.
