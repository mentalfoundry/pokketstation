...
A PocketStation app extracted as a single PS1 save (most save managers export these as `.mcs`) is this same Title Sector body, just prefixed with one real PS1 directory frame (`0x80` bytes: in-use marker, little-endian data size at `0x04`, link/filename fields). `psemu_load_mcs` strips that frame and validates the size against it, then defers to the same Title Sector loading `psemu_load_app` does - both frontends try `.pss` first and fall back to `.mcs` regardless of the file's actual extension.
