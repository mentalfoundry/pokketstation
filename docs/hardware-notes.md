...
- **Unresolved: a real IR-using app's transmitted pulses come out ~1% too short in this emulator, and it is not yet known why.** The app arms Timer2 with its nominal pulse unit minus a hardcoded 184 (1200 − 184 = 1016), and its receive handler accepts a sync pulse only within `4×unit ± unit/2`, i.e. `[4200, 5400]` Timer2 ticks. This emulator produces 4162. That is short by 38, just outside the window, so no transfer ever decodes.
  ...
  **Resolved by disassembly: two real emulator bugs, and a diagnostic that had been measuring the wrong memory entirely.** Working from the addresses actually observed executing, rather than from guessed field offsets, produced three findings:
  ...
  Together these take the receiver from decoding nothing at all to advancing through sync into bit accumulation and assembling a complete 328-bit message. A snapshot of each side's buffer, taken at the moment its own `bit_index` peaks (the buffers are cleared and reused afterwards, so an end-of-run read finds only zeroes and proves nothing), currently agrees on 24 of 41 bytes with both sides holding real non-zero content. That is not yet a correct transfer, and the remaining difference is the open question. Note the two snapshots are taken at different frames and possibly different table indices, so part of the difference may be an artifact of comparing the wrong pair of buffers rather than a decode error.
