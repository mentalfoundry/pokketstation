...
- **Unresolved: a real IR-using app's transmitted pulses come out ~1% too short in this emulator, and it is not yet known why.** The app arms Timer2 with its nominal pulse unit minus a hardcoded 184 (1200 − 184 = 1016), and its receive handler accepts a sync pulse only within `4×unit ± unit/2`, i.e. `[4200, 5400]` Timer2 ticks. This emulator produces 4162. That is short by 38, just outside the window, so no transfer ever decodes.
  ...
  **Cause 2 (found, not yet fixed): the two instances' independently app-controlled CPU clock speeds drift apart before a transfer starts, and the receiver never recovers alignment.** `CLK_MODE` is written by the app itself, not by any emulator power heuristic. In this scenario the receiving instance raises its own clock to its fastest mode (`~4MHz`) as soon as it arms to receive, while the sending instance stays at a slower mode (`~254KHz`) for tens of frames until just before it actually transmits. Because a fixed number of raw CPU cycles represents very different amounts of real wall-clock time at those two different clock speeds, the receiver's own reference clock accumulates roughly 1.57 million reference-cycle units of drift relative to the sender's before both sides finally settle on the same clock speed - and that drift persists for the rest of the run, since nothing re-synchronizes it. Even after Cause 1's fix delivers every edge, the receiver's decode still fails identically (same final state, same frame), which isolates this as the remaining cause: complete edge delivery is not enough if the timeline the edges are measured against is already off by that much. This is exactly the kind of divergence two real, separately-oscillator-clocked PocketStations would not experience together, which makes it likely specific to how two emulator instances are paced against each other, not a property of the real protocol. Re-synchronizing the two instances' reference clocks when their `CLK_MODE`s diverge, or pacing relay by real wall-clock time rather than raw cycle count (as the planned two-process IR Link frontend already intends to, using the host's own wall clock, not raw cycles), is the next real lead.
