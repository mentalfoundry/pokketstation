...
- **Command `0x58` (Get Status)** returns FLAG and three bytes: the data length `0x02`, then two status bytes `0x01` each. The BIOS handler at `0x04000B52` always returns those values. It does not read any RAM byte to produce the reply. An app does not install a FIQ hook for this command: a run of 100 docked frames after a correct `0x59` exchange produces the same reply. The `launch58` mode of `com_probe` confirms this.

- **Command `0x59` (Prepare File Execution)** takes 9 bytes. The PS1 sends `0x81`, `0x59`, a dummy zero, the dir_index high byte, the dir_index low byte, and four exec_parameter bytes. Byte 2 must be zero: the BIOS reads it and exits the handler after returning the data length (`0x06`) when it is nonzero. A dir_index from `0x0000` to `0x000F` starts the file at that slot. The kernel writes the index to `RAM[0xD0]`, ComFlags bit 11 (start_file) becomes 1, and the app runs on the next CPU frame. A dir_index from `0x0010` to `0xFFFD` is a no-op. The `launch58` mode of `com_probe` confirms this behavior.
