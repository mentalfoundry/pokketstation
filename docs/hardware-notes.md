...
- **Unresolved: a real IR-using app's transmitted pulses come out ~1% too short in this emulator, and it is not yet known why.** The app arms Timer2 with its nominal pulse unit minus a hardcoded 184 (1200 − 184 = 1016), and its receive handler accepts a sync pulse only within `4×unit ± unit/2`, i.e. `[4200, 5400]` Timer2 ticks. This emulator produces 4162. That is short by 38, just outside the window, so no transfer ever decodes.
  ...
  **A direct byte-for-byte comparison of the transmitter's and receiver's data buffers (`tools/ir_probe.c`) shows this gets a real 41-byte message through almost entirely correctly: 37 of 41 bytes decode exactly, reproducibly, across repeated runs.** The remaining 4 bytes are wrong in a specific, deterministic way (a real app's own encoding always losing specific "1" bits, never gaining a spurious one), not a marginal timing failure: the same 4 bytes fail identically at two different stretch values (200 and 316) and at two very different relay granularities (200 cycles and 1 raw cycle), which rules out both `IR_TX_FALL_STRETCH_CYCLES`'s own magnitude and `tools/ir_probe.c`'s relay-slice jitter as the cause. Disassembly located the real per-bit decode threshold (distinct from the sync-pulse check, and using a `-224`/`+224` tick correction this project has not yet connected to anything else measured) but not yet the exact runtime path that reaches it for this message. This is the most promising lead for closing the remaining gap.
