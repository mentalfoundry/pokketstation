...
Next step: with real INTC-access tracing now available (`intctrace` flag), correlate exactly which instructions run in the ~30-90 instruction window right after each `0x04003784` (button-action handler) entry, to find where the RAM `[r4+0x24]` action-dispatch value (confirmed via a runtime watchpoint to never change from 0 under simulated presses) should be getting written, and why it isn't.
