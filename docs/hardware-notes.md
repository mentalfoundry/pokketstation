...
**Interrupt controller** (`0x0A000000`): `hold`, `status`, `enable`, `mask` registers; `intc_irq_asserted`/`intc_fiq_asserted` compute `hold & enable & INT_IRQ_MASK`/`INT_FIQ_MASK` on demand. Button presses and the RTC tick assert into `status` (`INT_STATUS_MASK` bits); everything else (timers, IR, COM, battery) asserts into `hold`. This part matches the real register semantics exactly (`INT_LATCH`/`INT_INPUT`/`INT_MASK_SET`/`INT_MASK_CLR`/`INT_ACK` map onto the same four registers) - no discrepancies found here.
...
None of these three fixes resolved the separate, still-open issue of the app-dispatch routine (`0x04001900-0x04001AFC`, see below) never being reached - confirmed by direct instrumentation before and after. They're independently-confirmed correctness fixes regardless.

### Open: the app-dispatch routine is never reached

Direct instrumentation (tracking every instruction executed in `0x04001900-0x04001AFC`, the full app-selection/dispatch function documented above) shows it is **never entered at all**, regardless of which real app's slot index is forced into RAM `0x00D0`, across runs up to 500M instructions. PC instead spends the entire run oscillating within a different BIOS region (`~0x04003xxx`), most likely the BIOS's own idle/menu-selection screen (LCD framebuffer activity was confirmed to contain real, non-blank pixel content - a blinking pattern at the same PC, consistent with a menu cursor or icon animation), which never calls into the dispatch function under the current simple periodic-Fire-button simulation used by `tools/inspect.c`. Next step: disassemble the `0x04003xxx` loop to determine what button/navigation sequence it's actually waiting for.
