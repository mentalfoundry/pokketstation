...
`0x0C800000`: `IRDA_MODE` (`+0x0`) bit0 `IFMODE` (0=Receive, 1=Transmit), bit1 `STDBY` (0=Active, 1=Stand-by), bit2 `BGEN` (0=Enable 40kHz carrier generator, 1=Disable), bit3 `BFLT` (0=Enable filter, 1=Disable). `IRDA_DATA` (`+0x4`) bit0 `LED`: in transmit, the value software bit-bangs (0=off, 1=on); in receive, this emulator's own read-back of the current demodulated level. That receive-side meaning is inferred, not confirmed. See below. `IRDA_MISC` (`+0xC`) is unknown/reserved: this emulator reads it back as 0 and ignores writes, the same stub treatment as `BATT_CTRL` below.

**Source: [psx-spx](https://psx-spx.consoledev.net/pocketstation/)**, the community-maintained PlayStation hardware reference (formerly Martin Korth's "nocash psx-spx"). This project's own IR model was built by disassembling a real app before this reference was consulted; the two independently agree on the register layout above, which raises confidence in both. psx-spx is itself community reverse-engineering, not a leaked Sony devkit spec, and marks several IR details "reportedly" or with a question mark. Its two mirrors even disagree with each other: psx-spx.consoledev.net gives `STDBY`/`BGEN`/`BFLT` as above, while the older problemkaputt.de mirror instead guesses a plain "disable" bit plus two "IR_SEND_READY"/"IR_RECV_READY" bits it calls uncertain outright. This project's disassembly is what actually settles that disagreement in favor of `STDBY`/`BGEN`/`BFLT` - a real app's behavior outranks either secondary source.

**What psx-spx does not have: any numeric timing.** No microsecond pulse widths, no carrier-to-pulse ratio beyond "long is usually twice as long as short", nothing resembling the ~184-tick transmit-timing gap this project's own hardware testing is chasing (see "Known open questions" below). A register reference cannot answer that; only measurement on real hardware can, which is why `pk_timing_bench` screens 6-8 exist.

**What psx-spx does confirm, independent of this project's own disassembly:**
- The real BIOS "doesn't contain any IR functions, aside from some basic initialization and power-down stuff". This matches what this project's own trace already found: a real app drives `IRDA_MODE`/`IRDA_DATA` directly from its own code, through its own hand-rolled interrupt handler, not through any documented BIOS SWI. There is no undiscovered BIOS-level IR API to go looking for.
- Real pulses alternate ON/OFF instead of holding one long ON period, because real IR receiver hardware adapts to ambient light: a sustained signal risks being read as the new ambient level instead of as data. This project's own transmit-side trace already shows exactly this alternating short/long shape (see `tools/ir_probe.c`).
- `INT_IRDA` triggers on rising or falling edges of incoming data. A real handler typically reads Timer 2's live counter (reload `0xFFFFh`) to measure the interval. This project inferred that same technique from disassembly; psx-spx now corroborates it independently.
...
- The `BFLT` debounce window (~2 carrier periods, ~50us) - not in psx-spx, and nothing in this project's traced corpus (over 200 million instructions) has ever touched these registers, so there is no real-hardware measurement to check this against either.
- `IRDA_DATA` reflecting the live demodulated level on read while in receive mode. Neither psx-spx nor any documentation this project has seen describes the receive-side meaning of this bit; only the transmit-side ("LED") meaning is documented.
...
- **IR (`0x0C800000`+)**: see "IR / IR Link" above for the full model, including what [psx-spx](https://psx-spx.consoledev.net/pocketstation/) does and does not confirm. The `BFLT` debounce window and the receive-side meaning of `IRDA_DATA` bit0 are both inferred, not confirmed: neither is in psx-spx, no trace in this project's test corpus (over 200 million instructions) has ever touched these registers, and there is no second real PocketStation available to validate a modeled protocol against. `IRDA_MISC` (`+0xC`) is psx-spx's own unknown/reserved register; this emulator stubs it (reads 0, writes ignored) rather than guessing at behavior neither source documents.
- **Unresolved: a real IR-using app's transmitted pulses come out ~1% too short in this emulator, and it is not yet known why.** The app arms Timer2 with its nominal pulse unit minus a hardcoded 184 (1200 − 184 = 1016), and its receive handler accepts a sync pulse only within `4×unit ± unit/2`, i.e. `[4200, 5400]` Timer2 ticks. This emulator produces 4162. That is short by 38, just outside the window, so no transfer ever decodes.
  ...
  Tracing the app showed what both screens missed: its transmit handler re-arms the timer on every interrupt instead of letting it free-run. That changes what latency does. A free-running timer holds its period however late the handler runs, so latency cancels. A re-armed timer does not start its next period until the handler reaches the re-arm, so latency is added to every period. That is what the 184 budgets for, and `pk_timing_bench` screen 8 now measures it directly: this emulator reaches the re-arm in 31 ticks.
