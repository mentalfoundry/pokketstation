...
**Status after implementing bank-select**: forcing RAM `0x00D0` to a chosen slot and committing a real bank now gets the CPU running genuine mixed ARM/Thumb app code well past the point of the earlier `CDP`-decode crash (which was exactly the missing-bank-select symptom it looked like). Execution reached a real Thumb `SWI` (`0xDF13` at a FLASH1 address) that the *app itself* issued as an actual syscall - confirmed by checking the raw bytes and correctly decoding it as Thumb. The next crash happens ~30 instructions after that syscall's return, at an address whose bytes only make sense as Thumb but which the CPU was executing in ARM mode - i.e. something in the SWI-return path (or the BIOS handler's own dispatch/jump after handling the syscall) isn't restoring/threading the Thumb bit correctly. Not yet root-caused; the BIOS's actual SWI vector handler (at `0x00000008`'s branch target) hasn't been disassembled yet - that's the next concrete lead.
