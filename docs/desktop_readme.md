...
- **Tools > Sound** holds two separate things: **Volume**, how loud the output is, and **Speaker**, how it is voiced.
- **Tools > Sound > Volume** sets this emulator's own output volume, from **Mute** through 10% steps to **Full** (the default). It is an ordinary application volume: it scales what this window sends to your sound device and the emulated PocketStation cannot see it, so it works on any BIOS, applies immediately, and persists in `settings.cfg`. It is separate from, and multiplies with, the PocketStation's own sound setting - a device muted in its system menu stays silent whatever this is set to. The emulated machine's own volume is set only from its own system menu, the way the real device works; this app no longer offers a way to pin it.

  The percentages are **loudness, not signal level**: 50% is meant to sound half as loud, 25% a quarter as loud, and so on. Because hearing is logarithmic, that is not the same as halving the samples - halving them is only -6 dB and still sounds around two thirds as loud. Each step down the menu is a real 10 dB per halving, so 50% is -10 dB and 10% is -33 dB.
- **Tools > Sound > Speaker** sets how the output is *voiced*, as opposed to how loud it is. A real PocketStation's speaker is about a centimetre across in a plastic shell, and reproduces almost nothing below roughly 1-2 kHz. The emulator hands your computer the signal at that speaker's terminals, and a laptop or desktop speaker happily reproduces all the low end the real device physically could not - which is why the raw output sounds thick and muddy next to hardware. This filters back out what the real speaker never produced:

  - **Full Range (raw DAC output)** - no filtering, exactly what the emulated DAC held.
  - **Light** - takes the boom off without changing the character much. Good on speakers that are already small, or if the other two sound too thin.
  - **PocketStation Speaker** (the default) - the device: the low end gone from about 1.1 kHz down, with the resonant peak a small transducer has.
  - **Tinny** - further than the hardware goes, for large speakers or a subwoofer where even the default still has more low end than the device ever had.

  It persists in `settings.cfg` and applies immediately, and like Volume above it is this application's own output processing, so it works on any BIOS and the emulated machine cannot see it. **All three filtered settings are around 4 dB quieter than Full Range.** That is not a bug to work around: the low end being removed is real energy, and a speaker that cannot produce it is genuinely quieter - the real device is too. Turn **Volume** up if you want it back.

  This is voiced by ear against how the hardware sounds, not measured off a real unit's speaker, so it is an approximation of the right *shape* rather than a hardware-confirmed response.
- **Tools > Date/Time Override** holds the PocketStation's clock setting at a value of your choosing. **Default** means the frontend does not interfere and the PocketStation's own system menus work normally; **OS Date/Time** tracks your computer's clock continuously. It persists in `settings.cfg`.

  Two things are worth knowing. **While the override is active, the PocketStation's own clock screen stops working** - you can still open it and press buttons, but the value will not stick, because the frontend is holding it. That is what makes the override an override. Switch the menu item back to **Default** to get the system screen working again.

  And **the override needs a BIOS this project has actually traced**. This setting lives in ordinary RAM, at an address found by tracing a real BIOS rather than from any published register map, so on an unrecognised BIOS revision the same address would mean something else. The menu greys out in that case rather than write and hope. See [hardware-notes.md](hardware-notes.md) for how the address was established.
